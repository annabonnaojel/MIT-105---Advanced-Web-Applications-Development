...
def add_qualification(request, trainer_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            qualification.save()
        return redirect('trainer_list')  # You can change this redirect to another page
    else:
...
def trainer_form(request):
    ...
    if request.method == "POST": 
        if "create" in request.POST:
            ...
            trainer = Trainer.objects.filter(Q(first_name__contains=search_query) |Q(middle_name__icontains=search_query) | Q(last_name__icontains=search_query) )
                                                 
        context = {"trainers": trainers, "search_query": search_query}
